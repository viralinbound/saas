-- ═══════════════════════════════════════════════════════════════════════════
-- 20260829180000_root_domain_in
-- The platform now lives on supershowroom.in — publish_store() builds the
-- branded host / URL on that domain. Body is otherwise identical to the
-- brand_only_subdomain version. Existing publications keep working because the
-- storefront resolves by the brand slug (subdomain / host_path), not the
-- stored URL string; re-publishing refreshes the URL.
-- ═══════════════════════════════════════════════════════════════════════════
...
values ('version', to_jsonb('2026.08.29-17_root_domain_in'::text))
